...
//	this->m_vec_pParticles.clear();
	...
	// Calculate how often we should create particles...
...
glm::quat cParticleEmitter::m_calcImposterRotationFromEye(glm::vec3 particlePos, glm::vec3 particleFront, glm::vec3 eye)
{
	glm::quat magicRotation = RotationBetweenVectors(particleFront, glm::normalize(eye - particlePos));
	
	return magicRotation;
}
...
//			   vec_pParticles.begin() );
		 ...
		 itPart != this->m_vec_pParticles.end(); itPart++)
	{
		...
		{
			glm::mat4 matRotation = glm::toMat4(pCurPart->qOrientation);
			glm::vec3 updatedFront = glm::normalize(glm::vec3(matRotation * glm::vec4(0.0f, 0.0f, 1.0f, 1.0f)));
			pCurPart->qOrientation = this->m_calcImposterRotationFromEye(pCurPart->location, updatedFront, eyePosition) * pCurPart->qOrientation;
		}
