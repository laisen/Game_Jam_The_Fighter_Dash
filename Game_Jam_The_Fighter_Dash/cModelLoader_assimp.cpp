...
bool cModelLoader::LoadModel_Assimp(std::string filename, cMesh& theMesh, std::string& errors)
{
	...
	{
		sVertex_xyz_rgba_n_uv2_bt_4Bones curVertex;
		...
		curVertex.v0 = pScene->mMeshes[meshIndex]->mTextureCoords[0][vertIndex].y;	// v

		curVertex.tx = pScene->mMeshes[meshIndex]->mTangents[vertIndex].x;
		curVertex.ty = pScene->mMeshes[meshIndex]->mTangents[vertIndex].y;
		curVertex.tz = pScene->mMeshes[meshIndex]->mTangents[vertIndex].z;

		curVertex.bx = pScene->mMeshes[meshIndex]->mBitangents[vertIndex].x;
		curVertex.by = pScene->mMeshes[meshIndex]->mBitangents[vertIndex].y;
		curVertex.bz = pScene->mMeshes[meshIndex]->mBitangents[vertIndex].z;
